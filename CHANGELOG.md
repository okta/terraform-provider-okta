...
* **`okta_user_group_memberships`**: Fixed create and read failing with `did not have expected group memberships after multiple checks` for users whose group list spans three or more pages (more than 400 groups). The pagination loop reused the same slice for every page, so decoding a later page overwrote the `*sdk.Group` values already collected from the previous page and hid those groups from the membership check. The resource was also silently dropped from state on refresh for the same reason. [#2951](https://github.com/okta/terraform-provider-okta/issues/2951)
