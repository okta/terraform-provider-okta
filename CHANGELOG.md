...
* **`okta_app_oauth`**: Fixed `client_secret` being silently rotated by Okta whenever an unrelated attribute (e.g. `redirect_uris`) was updated. The provider used to unconditionally resend whatever `client_secret` it had cached in state on every update, regardless of what actually changed; if the live secret had since been regenerated directly in Okta (e.g. via "Generate new client secret" in the Admin Console), the next apply would silently revert it back to the stale cached value, with no visible plan diff since `client_secret` is computed-only. The provider no longer resends the cached `client_secret` on update - only `client_basic_secret`/`client_basic_secret_wo` are ever sent, and only when explicitly set.
