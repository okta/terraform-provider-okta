...
## Upgrading from `role_type`

This resource was migrated from the Terraform SDKv2 to the Terraform Plugin Framework. As part of the migration, the `role_type` attribute was renamed to `role_ref`, and `status` is now a read-only computed attribute rather than a configurable input. Manage the subscription state directly: create the resource to subscribe, and destroy it to unsubscribe. Update existing configurations as follows:
