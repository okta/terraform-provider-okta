...
```terraform
resource "okta_oauth2_v1_clients_role_workflows_admin" "example" {
  client_id = "<client_id>"
...
- `type` (String) Discriminator field identifying the variant type. Must be set to "WORKFLOWS_ADMIN".
...
```shell
terraform import okta_oauth2_v1_clients_role_workflows_admin.example <client_id>/<id>
```
