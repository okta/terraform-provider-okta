...
Set `dpop_bound_access_tokens = true` to require DPoP for this client. The
authorization server then rejects token requests from the client that don't
carry a `DPoP` header.

The attribute is `Optional` and `Computed`. Leaving it out of the config means
the provider tracks whatever the org reports rather than forcing it to `false`,
so a value set outside of Terraform is not clobbered on the next apply. To
manage the setting as code, declare it explicitly:
...
~> When `dpop_bound_access_tokens` is `true` the Okta API does not allow
`client_credentials` or `implicit` in `grant_types`.
...
### If the client secret is regenerated directly in Okta
...
import`) never returns it. Because of this, the provider never resends the
`client_secret` it has cached in state back to Okta on an update; only the
explicit `client_basic_secret`/`client_basic_secret_wo` attributes are ever
sent. This means that if the client secret is regenerated directly in Okta
(for example, via "Generate new client secret" in the Admin Console) outside
of Terraform, a later `terraform apply` for unrelated attributes will not
revert it back to the value Terraform has cached in state - the live secret
in Okta is left untouched. `client_secret` in state will simply be stale
until the next time Terraform's own Create/Update response updates it (or
until you run `terraform apply` with `client_basic_secret`/
`client_basic_secret_wo` set to the new value).
