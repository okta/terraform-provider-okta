...
### With auth initiated from either the app or Okta
...
resource "okta_app_oauth" "example" {
  ...
  grant_types    = ["authorization_code"]
  redirect_uris  = ["https://example.com/"]
  login_uri      = "https://example.com/oauth/login/okta-openidconnect/"
  response_types = ["code"]
  login_mode     = "SPEC"
  login_scopes   = []
  hide_web       = false
...
- `hide_ios` (Boolean) Do not display application icon on mobile app. Can only be set to `false` when `login_mode` is NOT `DISABLED`
- `hide_web` (Boolean) Do not display application icon to users. Can only be set to `false` when `login_mode` is NOT `DISABLED`
...
- `login_mode` (String) The type of Idp-Initiated login that the client supports, if any. Valid values: `DISABLED`, `SPEC`, `OKTA`. Default is `DISABLED`.
- `login_scopes` (Set of String) List of scopes to use for the request. Valid values: `openid`, `profile`, `email`, `address`, `phone`. Required when `login_mode` is NOT `DISABLED`.
- `login_uri` (String) URI that initiates login. Required when login_mode is NOT `DISABLED`.
