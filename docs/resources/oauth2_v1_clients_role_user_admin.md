...
```terraform
resource "okta_oauth2_v1_clients_role_user_admin" "example" {
  client_id = "<client_id>"
  type      = "USER_ADMIN"
}
```
...
- `type` (String) Discriminator field identifying the variant type. Must be set to "USER_ADMIN".
...
```shell
terraform import okta_oauth2_v1_clients_role_user_admin.example <client_id>/<id>
```
