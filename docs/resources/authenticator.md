...
Different authenticator types have different provider configuration requirements:

### RADIUS Configuration (On-Premises MFA)
Only required for **`onprem_mfa`** authenticators. The following fields are mandatory:
- `provider_hostname` - Server host name or IP address
- `provider_auth_port` - The RADIUS server port (e.g., 1812)
- `provider_shared_secret` - Authentication key shared between RADIUS client and server
- `provider_user_name_template` - Username template expected by the provider

### DUO Configuration
Required when using **`duo`** authenticators with `provider_type` set to "DUO":
- `provider_host` - The Duo Security API hostname
- `provider_integration_key` - The Duo Security integration key
- `provider_secret_key` - The Duo Security secret key
- `provider_user_name_template` - Username template expected by the provider

### Other Authenticators
Most authenticators (including `webauthn`, `google_otp`, `okta_verify`, etc.) do not require provider configuration fields. Configuration is handled through the `settings` field instead.

### Using provider_json
When using `provider_json` for expressive provider configuration, individual `provider_xxx` fields are not required or validated. The `provider_json` field conflicts with other `provider_xxx` arguments and allows for complex provider configurations as documented in the [Okta Authenticators API](https://developer.okta.com/docs/reference/api/authenticators-admin/).
...
- `provider_auth_port` (Number) The RADIUS server port (for example 1812). This is defined when the On-Prem RADIUS server is configured. Required only for `onprem_mfa` authenticators. Conflicts with `provider_json` argument.
- `provider_host` (String) (DUO specific) - The Duo Security API hostname. Conflicts with `provider_json` argument.
- `provider_hostname` (String) Server host name or IP address. Required only for `onprem_mfa` authenticators. Conflicts with `provider_json` argument.
- `provider_integration_key` (String) (DUO specific) - The Duo Security integration key.  Conflicts with `provider_json` argument.
- `provider_json` (String) Provider JSON allows for expressive provider values. This argument conflicts with the other 'provider_xxx' arguments. The [CreateProvider](https://developer.okta.com/docs/reference/api/authenticators-admin/#request) illustrates detailed provider values for a Duo authenticator. [Provider values](https://developer.okta.com/docs/reference/api/authenticators-admin/#authenticators-administration-api-object) are listed in Okta API.
- `provider_secret_key` (String) (DUO specific) - The Duo Security secret key.  Conflicts with `provider_json` argument.
- `provider_shared_secret` (String, Sensitive) An authentication key that must be defined when the RADIUS server is configured, and must be the same on both the RADIUS client and server. Required only for `onprem_mfa` authenticators. Conflicts with `provider_json` argument.
- `provider_user_name_template` (String) Username template expected by the provider. Required for `onprem_mfa` and `duo` authenticators. Conflicts with `provider_json` argument.
