...
- `property_name` (String) The IdP property to match the evaluated expression against when `selection_type` is `DYNAMIC`. Maps to `actions.idp.matchCriteria[0].propertyName` in the API.
- `provider_expression` (String) Expression used to dynamically select an IdP. Only applicable when `selection_type` is `DYNAMIC`. Maps to `actions.idp.matchCriteria[0].providerExpression` in the API.
- `selection_type` (String) Determines how the IdP is selected. One of: `SPECIFIC`, `DYNAMIC`. Default: `SPECIFIC`
