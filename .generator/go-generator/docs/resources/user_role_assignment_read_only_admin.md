---
page_title: "okta_user_role_assignment_read_only_admin Resource - terraform-provider-okta"
description: |-
  Manages an Okta User Role Assignment Read Only Admin resource.
---

# okta_user_role_assignment_read_only_admin (Resource)

Manages an Okta User Role Assignment Read Only Admin.

## Example Usage

```hcl
resource "okta_user_role_assignment_read_only_admin" "example" {
  user_id = "<user-id>"
  type = "<type>"
...
```shell
terraform import okta_user_role_assignment_read_only_admin.example <user_id> <id>
```
