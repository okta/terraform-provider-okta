...
func TestAccResourceOktaAppGroupAssignments_1088_unplanned_changes(t *testing.T) {
	...
	bookmarkApp := fmt.Sprintf("%s.test", resources.OktaIDaaSAppBookmark)
	groupA := fmt.Sprintf("%s.a", resources.OktaIDaaSGroup)
	groupB := fmt.Sprintf("%s.b", resources.OktaIDaaSGroup)
	groupC := fmt.Sprintf("%s.c", resources.OktaIDaaSGroup)
