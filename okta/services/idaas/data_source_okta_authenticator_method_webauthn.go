...
var aaguidGroupDataSourceObjectType = types.ObjectType{
	AttrTypes: map[string]attr.Type{
		"name":    types.StringType,
		"aaguids": types.ListType{ElemType: types.StringType},
	},
}
...
func (d *authenticatorMethodWebauthnDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"aaguid_groups": schema.ListNestedAttribute{
				Computed:    true,
...
func (d *authenticatorMethodWebauthnDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if settings != nil {
		...
		for _, g := range settings.AaguidGroups {
			...
			aaguidsList, diags := types.ListValueFrom(ctx, types.StringType, aaguidValues)
		...
		aaguidGroupsList, diags := types.ListValueFrom(ctx, aaguidGroupDataSourceObjectType, groups)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		data.AaguidGroups = aaguidGroupsList
	} else {
		...
		data.AaguidGroups = types.ListNull(aaguidGroupDataSourceObjectType)
	}
