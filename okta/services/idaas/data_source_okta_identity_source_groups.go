...
func (d *identitySourceGroupsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	group_or_external_id := state.ExternalId.ValueString()

	client := d.Config.OktaIDaaSClient.OktaSDKClientV6()
	result, httpResp, err := client.IdentitySourceAPI.GetIdentitySourceGroup(ctx, identity_source_id, group_or_external_id).Execute()
	if err != nil {
		if httpResp != nil && httpResp.StatusCode == http.StatusNotFound {
			resp.Diagnostics.AddError(
				"Identity source group not found",
				fmt.Sprintf("No group with external ID %q found in identity source %q.", group_or_external_id, identity_source_id),
			)
			return
		}
		resp.Diagnostics.AddError(
