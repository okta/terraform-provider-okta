...
func (r *grantResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	createGrantResp, _, err := r.OktaGovernanceClient.OktaIGSDKClientV5().GrantsAPI.CreateGrant(ctx).GrantCreatable(*buildGrant(data)).Execute()
	if err != nil {
		return
	}
...
func (r *grantResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	updatedGrantResp, _, err := r.OktaGovernanceClient.OktaIGSDKClientV5().GrantsAPI.UpdateGrant(ctx, plan.Id.ValueString()).GrantPatch(buildGrantPatch(plan)).Execute()
	if err != nil {
