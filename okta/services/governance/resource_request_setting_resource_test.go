...
func TestAccRequestSettingResource_basic(t *testing.T) {
	mgr := newFixtureManager("resources", resources.GovernanceRequestSettingResource, t.Name())
	config := mgr.GetFixtures("basic.tf", t)
	updatedConfig := mgr.GetFixtures("updated.tf", t)
	resourceName := fmt.Sprintf("%s.test", resources.GovernanceRequestSettingResource)
