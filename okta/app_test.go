...
func TestAppUpdateStatus(t *testing.T) {
	...
	testAppSchema := buildAppSchema(map[string]*schema.Schema{
		"test": {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			additionalChanges, err := appUpdateStatus(context.Background(), d, m)
			require.NoError(t, err)
...
func TestAppUpdateStatus_all_apps(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			resource:     appSecurePasswordStore,
			attrName:     "url",
