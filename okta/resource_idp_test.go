...
func TestAccSocialIdp(t *testing.T) {
	ri := acctest.RandInt()
	mgr := newFixtureManager(socialIdp)
	config := mgr.GetFixtures("basic.tf", ri, t)
	fbName := fmt.Sprintf("%s.facebook", socialIdp)
	microName := fmt.Sprintf("%s.microsoft", socialIdp)
	googleName := fmt.Sprintf("%s.google", socialIdp)
