...
func frameworkListApps(ctx context.Context, config *Config, filters *appFilters, limit int64) ([]okta.ListApplications200ResponseInner, error) {
	...
	apps, resp, err := config.oktaSDKClientV5.ApplicationAPI.ListApplications(ctx).Execute()
	if err != nil {
		return nil, err
	}
	resultingApps := make([]okta.ListApplications200ResponseInner, len(apps))
	for i := range apps {
		resultingApps[i] = apps[i]
	}
